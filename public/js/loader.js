if(self==top){(function(){
	...
	var visit_matched = function(reg_arr) {
	...
	data.visit_matched = visit_matched;
	data.root_prefix = 'http://dynamic.appgame.com/';

	if (visit_matched(['-homemini$'])) {
		console.log('bypass url: '+window.location.href);
		return;
	}
	
	if (visit_matched(['mp.weixin.qq.com'])) {
		console.log('weixin mode');
		LazyLoad.js( data.root_prefix+'js/jquery.min.js', function() {
			window.jQomp = jQuery.noConflict(true);
			LazyLoad.js(data.root_prefix+'cache/identify.js', function() {
				LazyLoad.js( data.root_prefix+'js/wx_main.js', function(){ 
					jQomp(omp_main);
				});
			});
		});
		...
		LazyLoad.js( data.root_prefix+'js/jquery.min.js', function() {
			...
			LazyLoad.js([ data.root_prefix+'js/pushstream.js',
			    ...
			    data.root_prefix+'cache/identify.js'
			], function() {
